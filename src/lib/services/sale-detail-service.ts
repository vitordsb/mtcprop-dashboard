...
function formatBooleanLabel(value: string | number | boolean | null | undefined) {
  if (value == null) {
    return null;
  }
...
export const saleDetailService = {
  async getOverview(saleId: string): Promise<SaleDetailOverview | null> {
    ...
    return unstable_cache(
      async (): Promise<SaleDetailOverview | null> => {
        const sale = await getGuruTransactionById(normalizedSaleId);
        ...
        const installmentsLabel =
          installmentsQty && installmentsValue != null
            ...
            : null;
        const installmentInterestLabel = formatCurrency(
          Number(raw?.payment?.installments?.interest ?? 0),
          currency,
        );

        const firstItem = raw?.items?.[0] ?? null;
        const productUnitValue = Number(firstItem?.unit_value ?? raw?.product?.unit_value ?? 0);
        const productTotalValue = Number(
          firstItem?.total_value ?? raw?.product?.total_value ?? sale.amount ?? 0,
        );
        ...
        return {
          ...
          sale: {
            ...
            discountValueLabel: formatCurrency(
              Number(raw?.payment?.discount_value ?? 0),
              currency,
            ),
            taxValueLabel: formatCurrency(Number(raw?.payment?.tax?.value ?? 0), currency),
            affiliateValueLabel: formatCurrency(
              Number(raw?.payment?.affiliate_value ?? 0),
              currency,
            ),
            marketplaceValueLabel: formatCurrency(
              Number(raw?.payment?.marketplace_value ?? 0),
              currency,
            ),
            ...
            invoiceActionLabel:
              raw?.invoice && !Array.isArray(raw.invoice) && raw.invoice.marketplace_id
                ? `Marketplace ${raw.invoice.marketplace_id}`
                : null,
            refundAvailable: Boolean(
              sale.statusCode &&
                !["canceled", "cancelled", "refunded", "chargeback"].includes(sale.statusCode),
            ),
            chargebackAvailable: Boolean(
              sale.statusCode && !["chargeback"].includes(sale.statusCode),
            ),
            buyer: {
            ...
            product: {
              ...
              marketplaceName:
                firstItem?.marketplace_name ?? raw?.product?.marketplace_name ?? null,
              marketplaceId:
                firstItem?.marketplace_id ?? raw?.product?.marketplace_id ?? null,
            ...
            extras: {
              ...
              acceptedTerms: formatBooleanLabel(raw?.extras?.accepted_terms_url),
              acceptedPrivacyPolicy: formatBooleanLabel(
                raw?.extras?.accepted_privacy_policy_url,
              ),
            },
          },
        };
      },
      [CACHE_TAGS.SALE_DETAIL, normalizedSaleId],
      {
        revalidate: 60,
        tags: [CACHE_TAGS.SALE_DETAIL, CACHE_TAGS.SALES_OVERVIEW],
      },
    )();
  },
};
